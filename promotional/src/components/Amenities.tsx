...
export default function Amenities() {
  ...
  return (
    <section id="amenities" className="py-28 px-6 lg:px-12" style={{ background: "var(--dark)" }}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px" style={{ background: "var(--border-color)" }}>
          {t.amenities.items.map((item, index) => (
            <div
              key={item.title}
              className="group p-10 transition-colors duration-500"
              style={{ background: "var(--dark)" }}
              onMouseEnter={(e) => (e.currentTarget.style.background = "var(--dark-mid)")}
              onMouseLeave={(e) => (e.currentTarget.style.background = "var(--dark)")}
            >
              <div className="mb-6 transition-colors duration-300 group-hover:opacity-80" style={{ color: "var(--gold)" }}>
                {icons[index]}
              </div>
              <h3 className="text-xl font-light mb-4" style={{ color: "var(--cream)", fontFamily: "var(--font-cormorant)" }}>
                {item.title}
              </h3>
              <p className="text-sm leading-7 opacity-60" style={{ color: "var(--cream)", fontFamily: "var(--font-raleway)", fontWeight: 300 }}>
                {item.description}
              </p>
            </div>
          ))}
